...
for (i in seq_along(dataset6)) {
  cat(
    dataset6[i],
    ": ",
    sum(map_dbl(xxx[[i]], ~ sum(.[(warmup + 1):dim(.)[1], "divergent__"]))) /
      ((iter - warmup) * 4),
    "\n"
  )
}
...
for (i in seq_along(dataset6)) {
  cat(
    dataset6[i],
    ": ",
    sum(map_dbl(xxy[[i]], ~ sum(.[(warmup + 1):dim(.)[1], "divergent__"]))) /
      ((iter - warmup) * 4),
    "\n"
  )
}
...
plot_data8 <- roc8 %>%
  ...
  mutate(
    ...
    response = factor(
      response,
      levels = c("4new", "3new", "2new", "1new", "1old", "2old", "3old", "4old")
    )
  )
...
plot_data_2 %>%
  ...
  geom_point(
    aes(x = gumbel_NEW, y = gumbel_OLD, shape = "Gumbel", colour = "Gumbel"),
    size = psize,
    stroke = ssize
  ) +
  geom_point(
    aes(x = uvsd_NEW, y = uvsd_OLD, shape = "UVSD", colour = "UVSD"),
    size = psize,
    stroke = ssize
  ) +
  geom_label(
    mapping = aes(x = 0.75, y = 0.15, label = n_text),
    data = bin_n,
    hjust = "center",
    vjust = "top",
    parse = TRUE,
...
#ggsave("confroc-plot-uvsd.pdf", width = 22, height = 17.25, units = "cm")
