uvsdt8agglog_stanvars <- "
  real uvsdt8agglog_lpmf(int y, real mu, real discsignal, real crc, 
  ...
  pold[nthres+1] = normal_lccdf(thres[nthres] | mu, disc); 
  ...
  pnew[nthres+1] = normal_lccdf(thres[nthres] | 0, 1);
  return multinomial_logit_lpmf(oldvec | pold) + 
      multinomial_logit_lpmf(newvec | pnew);
  }
"

uvsdt8agglog_family <- custom_family(
  name = "uvsdt8agglog", 
...
sv_uvsdt8agglog <- stanvar(scode = uvsdt8agglog_stanvars, block = "functions")

calc_posterior_predictions_uvsdt8agglog <- function(i, prep) {
  ...
  pold[,nthres+1] = 1 - pnorm(thres[,nthres], mu, discsignal)
...
log_lik_uvsdt8agglog <- function(i, prep) {
  use <- calc_posterior_predictions_uvsdt8agglog(i = i, prep = prep)
...
posterior_epred_uvsdt8agglog <- function(prep) {
  nobs <- prep$nobs
  ...
  for (i in seq_len(nobs)) {
    tmp <- calc_posterior_predictions_uvsdt8agglog(i = i, prep = prep)
    out[,i,] <- c(tmp$pold, tmp$pnew)
  }
  return(out)
}

posterior_predict_uvsdt8agglog <- function(i, prep, ...) {
  use <- calc_posterior_predictions_uvsdt8agglog(i = i, prep = prep)
  oldvec <- c(prep$data$Y[i], prep$data$vint1[i], prep$data$vint2[i], prep$data$vint3[i], 
              prep$data$vint4[i], prep$data$vint5[i], prep$data$vint6[i], prep$data$vint7[i])
  newvec <- c(prep$data$vint8[i], prep$data$vint9[i], prep$data$vint10[i], prep$data$vint11[i], 
