...
d <- d %>% 
  mutate(correct_face = case_when(
    correctanswer == 1 ~ face1,
    correctanswer == 2 ~ face2,
    correctanswer == 3 ~ face3,
    correctanswer == 4 ~ face4
  )) %>% 
  mutate(all_face = str_c(face1, face2, face3, face4))

head(d)

unique(c(d$face1, d$face2, d$face3, d$face4))
unique(d$correct_face)
length(unique(d$all_face))
unique(d$eprime.levelname)
with(d, table(correct_face, eprime.levelname))
...
spw1<-pw %>% filter(response %in% c(1,2)) %>% group_by(subject,acc,time_point,trial_type) %>% summarise(pitch=mean(pitch))

the_counts <- pw %>% filter(time_point==1,response %in% c(1,2) ) %>% group_by(subject,trial_type) %>% summarise(count=n()) 

the_counts %>% group_by(trial_type) %>% summarise(min(count),
                                                  max(count),
                                                  mean(count)) %>% print()

##ACCURACY IN ACOUSTIC DATA FILTERED IN SAME MANNER---------------------------
behav_acc <- pw %>% filter(time_point==1,response %in% c(1,2) ) %>% group_by(subject,trial_type) %>% summarise(prop=mean(acc)) 

behav_acc %>% filter(trial_type=='FourAFCProc') %>% ungroup() %>%
  t_test(prop~1,mu=.25)
behav_acc %>% filter(trial_type=='FourAFCProc') %>% ungroup() %>%
  cohens_d(prop~1,mu=.25)
behav_acc %>% filter(trial_type=='FourAFCProc') %>% ungroup() %>%
...
o1_trial_type <- o1 %>% filter(Effect=='trial_type') %>% mutate(padj=p.adjust(p,'fdr'),
                                                                sigaa =ifelse(padj<=.05,1,NA)) 

#sigaa is a logical indicating if the adjusted p value remains significant

o1_trial_type %>% filter(sigaa==1) %>% print()

o1_acc <- o1 %>% filter(Effect=='acc') %>% mutate(padj=p.adjust(p,'fdr'),
                                                  sigaa =ifelse(padj<=.05,1,NA)) 
o1 %>% filter(Effect=='acc:trial_type') %>% mutate(padj=p.adjust(p,'fdr')) %>% 
  filter(padj<=.050) %>% print()
...
windows(8,6) #plotting trial type effect
p1b <- ggplot(left_join(spw1,dplyr::select(o1_trial_type,time_point,sigaa)),
              aes(x=time_point,y=pitch,color=trial_type,group=trial_type)) +
  #stat_summary(fun = mean, color = "red", geom = "line") +
  stat_summary(fun.data = 'mean_se',
               fun.args = list(mult = 1),
               geom = 'smooth', se = TRUE,size=2)+
  geom_point(aes(x=time_point,y=sigaa*150),size=4,color='black')+
  #geom_point(aes(x=time_point,y=ps*325),size=4,color='black')+
  theme_minimal(base_size = 18)+
  theme(legend.position = c(.4, .9))+
  annotate(geom="text", x=0, y=475, label="b)",size=8)+
  xlab('time bin (1-20)')+
  scale_color_manual(values = c('firebrick3','seagreen'))
print(p1b)

#pitch plot
windows(15,6)
grid.arrange(p1,p1b,nrow=1)

####OMNIBUS COLLAPSED ACROSS TIME-----------------------------------------------
spw2<-pw %>% filter(response %in% c(1,2)) %>% group_by(subject,acc,trial_type ) %>% summarise(sdpitch=sd(pitch),
                                                                                              mnpitch=mean(pitch))
...
spw2 %>% group_by(subject,trial_type) %>% summarise(mnpitch=mean(mnpitch),.groups = 'drop') %>%  t_test(mnpitch~trial_type,paired=T)
spw2 %>% group_by(subject,trial_type) %>% summarise(mnpitch=mean(mnpitch),.groups = 'drop') %>% cohens_d(mnpitch~trial_type,paired=T)
spw2 %>% group_by(subject,trial_type) %>% summarise(mnpitch=mean(mnpitch),.groups = 'drop') %>% 
  pivot_wider(names_from = trial_type,values_from = mnpitch) %>% cor_test(vars = c(FourAFCProc,TwoAFCProc))
...
a2b <- ezANOVA(data = spw2 %>% mutate(acc=as.factor(acc)), 
  ...
  get_pes() %>% select(Effect,DFn,DFd,F,p,pes) %>% filter(Effect!='(Intercept)') %>%
  mutate_if(is.double,round,3)  %>% pastey() %>% print()
#the deviation of pitch differs for trial type, but not accuracy.


spw2 %>% group_by(trial_type) %>%
...
#########NOW INTENSITY------------------------------------------------
siw1<-iw %>% filter(response %in% c(1,2)) %>% group_by(subject,acc,time_point,trial_type) %>% summarise(intensity=mean(intensity))
# 
# t_2 <- siw1 %>% group_by(time_point) %>%
#   t_test(intensity ~ acc, paired = T,detailed = F)
# fdr_ps_i<-data.frame(ps = as.numeric((p.adjust(t_2$p, method = 'fdr', n = length(t_2$p)) <= .05)),
#                    time_point = 1:20) %>% mutate(ps = ifelse(ps==0,NA,ps))
# 
# siw1<-left_join(siw1,fdr_ps_i) %>% left_join(.,s1) %>% ungroup() %>% mutate(accgrp=ifelse(subacc>=median(subacc),'high','low'))
...
o2_trial_type <- o2 %>% filter(Effect=='trial_type') %>% mutate(padj=p.adjust(p,'fdr'),
                                                          sigaa =ifelse(padj<=.05,1,NA)) 
o2_trial_type %>% filter(sigaa==1) %>% print()
...
o2 %>% filter(Effect=='acc:trial_type') %>% mutate(padj=p.adjust(p,'fdr')) %>%
  filter(padj<=.050) %>% print()
...
windows(8,6)
p2b <- ggplot(left_join(siw1,dplyr::select(o2_trial_type,time_point,sigaa)),
              aes(x=time_point,y=intensity,color=trial_type,group=trial_type)) +
  #stat_summary(fun = mean, color = "red", geom = "line") +
  stat_summary(fun.data = 'mean_se',
               fun.args = list(mult = 1),
               geom = 'smooth', se = TRUE,size=2)+
  ...
  annotate(geom="text", x=0, y=58, label="b)",size=8)+
  xlab('time bin (1-20)')+
  ylab('loudness dB')+
  scale_color_manual(values = c('firebrick3','seagreen'))
print(p2b)
...
####OMNIBUS COLLAPSED ACROSS TIME-----------------------------------------------
siw2<-iw %>% filter(response %in% c(1,2)) %>% group_by(subject,acc,trial_type ) %>% summarise(sdintensity=sd(intensity),
...
siw2 %>% group_by(subject,trial_type) %>% summarise(mnintensity=mean(mnintensity)) %>% ungroup() %>%
  t_test(mnintensity~trial_type,paired=T) %>% print()
siw2 %>% group_by(subject,trial_type) %>% summarise(mnintensity=mean(mnintensity)) %>% ungroup() %>%
  cohens_d(mnintensity~trial_type,paired=T) %>% print()
siw2 %>% group_by(trial_type) %>% summarise(mnintensity=mean(mnintensity)) %>% ungroup() %>%
  glimpse()
...
siw2 %>% group_by(subject,trial_type) %>% summarise(sdintensity=mean(sdintensity)) %>% ungroup() %>%
  t_test(sdintensity~trial_type,paired=T) %>% print()
siw2 %>% group_by(subject,trial_type) %>% summarise(sdintensity=mean(sdintensity)) %>% ungroup() %>%
  cohens_d(sdintensity~trial_type,paired=T) %>% print()
siw2 %>% group_by(trial_type) %>% summarise(mean(sdintensity),
                                            sd(sdintensity)) %>% ungroup() %>%
  glimpse()
...
s2b <- pw %>% filter(response %in% c(1,2)) %>% filter(time_point==1)
#now integrate onset offset data.
s2b<-left_join(s2b,speech_onset_offset)

#now simplify to mean onset as below.....
s2b<- s2b %>% group_by(subject,trial_type, acc) %>% summarise(onset=mean(onset,na.rm=T)) %>%
  mutate(acc=as.factor(acc))

a2b <- ezANOVA(data = s2b,
...
s2b %>% group_by(subject,trial_type) %>% summarise(onset = mean(onset)) %>% ungroup() %>%
  t_test(onset~trial_type,paired=T) %>% print()
s2b %>% group_by(subject,trial_type) %>% summarise(onset = mean(onset)) %>% ungroup() %>%
  cohens_d(onset~trial_type,paired=T) %>% print()
s2b %>% group_by(trial_type) %>% summarise(mean(onset),
                                          sd(onset)) %>% print()

 
#SPEECH DURATION...I put it in both files so it doesn't matter which is used--------------------------------------------
s2 <- pw %>% filter(response %in% c(1,2)) %>% group_by(subject,trial_type, acc) %>% summarise(duration=mean(response_dur)) %>%
  mutate(acc=as.factor(acc))
...
s2 %>% group_by(subject,trial_type) %>% summarise(duration = mean(duration)) %>% ungroup() %>%
  t_test(duration~trial_type,paired=T) %>% print()
s2 %>% group_by(subject,trial_type) %>% summarise(duration = mean(duration)) %>% ungroup() %>%
  cohens_d(duration~trial_type,paired=T) %>% print()
s2 %>% group_by(trial_type) %>% summarise(mean(duration),
                                   sd(duration)) %>% print()

#summary stats for justin presentation
s2 %>% group_by(subject,acc) %>% summarise(duration = mean(duration)) %>%
  group_by(acc) %>%
  get_summary_stats(duration) %>% print()
s2 %>% group_by(subject,trial_type) %>% summarise(duration = mean(duration)) %>%
  group_by(trial_type) %>%
  get_summary_stats(duration) %>% print()
